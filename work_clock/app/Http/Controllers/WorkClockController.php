    		...
    		$item['day_type'] = $item['day_of_week'] >= 6 ? 0 : 1;
 			...
 			if(!key_exists($key, $data)){
				if(in_array($dayOfWeek, [0,6])){
 				...
 				if(date('H',(strtotime($theDayData[0]))<9)){
                    ...
                    $addTime = count($theDayData) == 2
 					...
 					$workTime = count($theDayData) == 2
						? number_format((strtotime($theDayData[1])-strtotime($theDayData[0]))/3600, 2) : 0;

                    (float)$workTime > 9.3 ? $subtime = 3600*9.5  : $subtime=0;
                    $addTime = count($theDayData) == 2
                        ? (int)((strtotime($theDayData[1])- $theDayData[0] - $subtime)/3600 ) . '.' . (int)(((strtotime($theDayData[1])- $theDayData[0] -$subtime)%3600)/60 ): 0;
 				}
